...
def recommend_patents(value,criteria,numRows):
    ...
    if criteria == 'Title':
        if value not in df['Title']: 
            ...
            mod_df = g_patent_data.append({'content':value},ignore_index=True)
            df = mod_df
        tfidf_des = vectorizer.fit_transform(foo(df['content'].values))
        indices = pd.Series(df['content'])
    elif criteria == 'Inventors':
    ...
    #print(similarity_scores)
      #Get the books index
    patents_index = [i[0] for i in similarity_scores if i[1]>0.05]
    #print (len(patents_index))
    if len(patents_index) == 0:
        print('No Matching Results Found')
    else:
    #Return the top 5 most similar patents using integar-location based indexing (iloc)
        return df[['Title','Patent Number','Inventors','Current Assignee','link']].iloc[patents_index]
